...
const AdminLogin: React.FC<Props> = ({ users, onLogin, onBack }) => {
  const [pid, setPid] = useState('');
  const [pass, setPass] = useState('');
  const [error, setError] = useState('');

  const handleLogin = () => {
    const user = users.find(u => u.personnelId === pid && u.password === pass && u.role === UserRole.ADMIN);
    if (user) {
      onLogin(user);
    } else {
  ...
  return (
    <div className="p-8 h-screen flex flex-col justify-center">
      ...
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-blue-900">ورود مدیران</h2>
        <p className="text-gray-500">اطلاعات مدیریتی خود را وارد کنید</p>
      </div>
